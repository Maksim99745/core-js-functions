...
function getPowerFunction(exponent) {
  const exponential = exponent;
  return (input) => {
    return input ** exponential;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  const partialFn = function closure(...nextArgs) {
    const allArgs = [...args1, ...nextArgs];
